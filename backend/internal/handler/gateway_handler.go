...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if platform == service.PlatformComposite {
		...
		if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelListingEnabled() {
			source := availableModels
			if len(source) == 0 {
				source = defaultModelIDsForPlatform(service.PlatformComposite)
			}
			writeAllowlistedModelsList(c, service.PlatformComposite, apiKey.Group.ModelListingAllowlist().FilterForListing(source))
			return
		}
	...
	if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelListingEnabled() {
		...
		writeAllowlistedModelsList(c, platform, apiKey.Group.ModelListingAllowlist().FilterForListing(source))
		return
	}
...
func (h *GatewayHandler) codexModelIDsForGroup(ctx context.Context, group *service.Group, platformOverride string) []string {
	...
	if platform == service.PlatformComposite {
		...
		if group.ModelListingEnabled() {
			source := availableModels
			if len(source) == 0 {
				source = fallbackModels
			}
			return group.ModelListingAllowlist().FilterForListing(source)
		}
		if len(availableModels) > 0 {
	...
	if group.ModelListingEnabled() {
		return group.ModelListingAllowlist().FilterForListing(modelListingSource(platform, availableModels, fallbackModels))
	}
	if len(availableModels) > 0 {
		return availableModels
	}
...
func (h *GatewayHandler) AntigravityModels(c *gin.Context) {
	models := antigravity.DefaultModels()
	if apiKey, ok := middleware2.GetAPIKeyFromContext(c); ok && apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelListingEnabled() {
		filtered := make([]antigravity.ClaudeModel, 0, len(models))
		for _, model := range models {
			if apiKey.Group.ModelListingAllowlist().Allows(model.ID) {
