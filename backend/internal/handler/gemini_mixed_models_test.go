...
func TestGeminiMixedModelListingPreservesDisplayOnlyConfiguration(t *testing.T) {
	for _, forced := range []bool{false, true} {
		t.Run(map[bool]string{false: "mixed", true: "forced"}[forced], func(t *testing.T) {
			id := int64(47)
			group := &service.Group{ID: id, Platform: service.PlatformGemini,
				ModelsListConfig: service.GroupModelsListConfig{Enabled: true, Models: []string{"gemini-visible"}},
			}
			repo := &geminiAllowlistAccountRepoStub{gatewayModelsAccountRepoStub: gatewayModelsAccountRepoStub{byGroup: map[int64][]service.Account{id: {
				{ID: 1, Platform: service.PlatformAntigravity, Extra: map[string]any{"mixed_scheduling": true}, Credentials: map[string]any{"model_mapping": map[string]any{"gemini-visible": "gemini-3.8-flash-high", "gemini-hidden": "gemini-3.8-flash-high"}}},
			}}}}
			h := &GatewayHandler{geminiCompatService: service.NewGeminiMessagesCompatService(repo, nil, nil, nil, nil, nil, nil, nil, nil)}
			rec := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(rec)
			c.Request = httptest.NewRequest(http.MethodGet, "/v1beta/models", nil)
			c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{GroupID: &id, Group: group})
			if forced {
				c.Set(string(middleware.ContextKeyForcePlatform), service.PlatformAntigravity)
			}
			h.GeminiV1BetaListModels(c)
			require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
			require.Contains(t, rec.Body.String(), "models/gemini-visible")
			require.NotContains(t, rec.Body.String(), "models/gemini-hidden")
			require.False(t, group.ModelAllowlistEnabled(), "display filtering must not enable request restrictions")
		})
	}
}
