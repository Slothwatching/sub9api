...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	filterGeminiModels := func(models []gemini.Model) []gemini.Model {
		if apiKey.Group == nil || !apiKey.Group.ModelListingEnabled() {
			return models
		}
		filtered := make([]gemini.Model, 0, len(models))
		for _, model := range models {
			if apiKey.Group.ModelListingAllowlist().Allows(model.Name) {
	...
	if apiKey.Group != nil && apiKey.Group.ModelListingEnabled() {
		if filtered, dropped, ok := filterUpstreamGeminiModelsBody(res.Body, apiKey.Group.ModelListingAllowlist()); ok && dropped {
