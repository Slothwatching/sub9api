...
func (r *usageLogRepository) getAllGroupUsageSummaryFromRollups(ctx context.Context, todayStart time.Time) (results []usagestats.GroupUsageSummary, err error) {
	// The watermark and aggregate must observe one snapshot. In READ COMMITTED,
	// a concurrent historical delete can invalidate the watermark between reads,
	// combining stale historical buckets with an already-deleted current tail.
	// Keep the upstream parameterized index boundary without splitting snapshots.
