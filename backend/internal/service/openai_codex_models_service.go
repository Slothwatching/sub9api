...
func FilterCodexModelIDsForGroup(modelIDs []string, group *Group) []string {
	...
	if group != nil && group.ModelListingEnabled() {
		for _, modelID := range group.ModelListingAllowlist().Models {
	...
	if group != nil && group.ModelListingEnabled() {
		filtered = group.ModelListingAllowlist().FilterForListing(filtered)
	}
	return filtered
}
...
func (s *OpenAIGatewayService) BuildGroupConfiguredCodexModelsManifest(
	...
	body, _, err = mergeConfiguredCodexModelsManifest(
		body,
		nil,
		group.ModelListingAllowlist().Models,
		group.ModelListingEnabled(),
	)
	if err != nil {
...
func (s *OpenAIGatewayService) MergeGroupConfiguredCodexModels(
	...
	if group.CodexModelsManifestConfig.Enabled && group.ModelListingEnabled() {
		body, err = orderPinnedCodexModelsBySelection(body, group.ModelListingAllowlist())
		if err != nil {
...
func openAIConfiguredCodexModelIDsForGroup(accounts []Account, group *Group) []string {
	...
	seen := make(map[string]struct{}, len(models)+len(group.ModelListingAllowlist().Models))
	for _, modelID := range models {
		seen[modelID] = struct{}{}
	}
	for _, selectedModel := range group.ModelListingAllowlist().Models {
