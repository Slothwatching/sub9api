          ...
          <template #cell-group="{ row }">
            <div class="group/dropdown relative">
              <button
                ...
                class="-mx-2 -my-1 flex max-w-full flex-wrap sm:flex-nowrap cursor-pointer items-center gap-2 rounded-lg px-2 py-1 transition-all duration-200 hover:bg-gray-100 dark:hover:bg-dark-700"
    ...
    >
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <fieldset v-if="!showEditModal" data-tour="key-form-provider">
          ...
          <div class="grid gap-2" :class="createProviderOptions.length === 1 ? 'grid-cols-1' : 'grid-cols-2 sm:grid-cols-4'">
            <label
            ...
            >
              ...
              >
                <span class="flex h-8 items-center justify-center gap-1.5" aria-hidden="true">
                  <span
                    v-for="platform in provider.platforms"
                    :key="platform"
...
const createProvider = ref<KeyGroupProvider>('openai')
const createProviderOptions = computed(() => KEY_GROUP_PROVIDERS.map((value) => ({
  ...
  platforms: [...new Set(groups.value
    .filter((group) => getKeyGroupProvider(group.platform) === value)
    .map((group) => group.platform))].slice(0, 2)
...
watch([showCreateModal, createProviderOptions], ([isOpen, providers], [wasOpen]) => {
  ...
  if (!wasOpen || !providers.some((provider) => provider.value === createProvider.value && provider.count > 0)) {
    selectCreateProvider(providers.find((provider) => provider.count > 0)?.value ?? 'openai')
  }
