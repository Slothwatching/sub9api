...
describe('user KeysView column settings', () => {
  ...
  describe('create provider selection', () => {
    ...
    it('defaults to an available provider and omits unavailable categories', async () => {
    ...
    it('shows only OpenAI when it is the only available provider', async () => {
      getAvailableGroups.mockResolvedValue([availableGroups[1]])
      const wrapper = await openCreate()
      const radios = wrapper.findAll<HTMLInputElement>('input[name="key-provider"]')
      expect(radios).toHaveLength(1)
      expect(radios[0].element.value).toBe('openai')
      expect(radios[0].element.checked).toBe(true)
      expect(optionIds(wrapper)).toEqual([2])
      expect(wrapper.get('[data-tour="key-form-provider"]').text()).not.toContain('keys.providers.anthropic')
    })
